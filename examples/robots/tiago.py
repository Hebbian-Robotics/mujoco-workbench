...
* **Strip the upstream single arm** (`arm_1_link` subtree) — the data-
  center scene replaces it with two Pipers attached via `examples/robots/piper.py`.
...
from examples.paths import TIAGO_XML
    ...
    for name in _TIAGO_REQUIRED_BODIES:
        if root.find("body", name) is None:
            raise RuntimeError(
                ...
                "update examples/robots/tiago.py if the new naming is intentional."
            )
...
def load_tiago(config: TiagoConfig = TiagoConfig()) -> mjcf.RootElement:
    ...
    what `examples/scenes/tiago_piper_server_cable_swap.py::build_scene` does.
...
def torso_world_pos_at_zero() -> tuple[float, float, float]:
    ...
    Referenced by `examples/scenes/tiago_piper_server_cable_swap_layout` so bin heights / IK targets
