...
* The Stanford Mobile ALOHA body silhouette (chassis + central column +
  ...
  four ViperX arms. See `examples.tools.strip_aloha_arms`.
...
* Two attachment SITES at the *exact* xy coordinates where the original
  ...
  `examples.tools.label_aloha_arms`) lived. The scene module attaches a
...
ARM #3 and #4 cluster centroids (from `examples.tools.label_aloha_arms`):
...
from examples.paths import MOBILE_ALOHA_STANFORD_BODY_STL
