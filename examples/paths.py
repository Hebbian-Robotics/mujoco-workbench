...
# Lazy menagerie-relative paths. `from examples.paths import UR10E_XML` triggers
...
# see `examples.tools.strip_aloha_arms`) leaving only the central rolling
...
def _mobile_aloha_mesh(filename: str) -> MenagerieMesh:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            "(see examples.tools.strip_aloha_arms)."
        )
    return MenagerieMesh(path)
