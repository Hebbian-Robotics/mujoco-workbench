...
Headless: uses `mujoco_workbench.runtime` helpers (which set `MUJOCO_GL=egl`
on Linux) so this runs over SSH without a display server. No viser involvement.
...
# `mujoco_workbench.runtime` does this too, but we duplicate it here because
# Python evaluates the `import mujoco` below before the runtime import
...
def main(
    ...
    # `mujoco_workbench.runtime.render_frame` allocates a fresh Renderer per call,
    ...
    # `initial_geom_rgba` / `restart()` logic in mujoco_workbench.runner.
    ...
    for camera_pass in cam_passes:
        ...
        ctx.scene.apply_initial_state(ctx.model, ctx.data, ctx.arms, ctx.cube_body_ids)
